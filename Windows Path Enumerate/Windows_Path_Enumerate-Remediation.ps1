...
Param (
    ...
    [parameter(Mandatory = $False,
        ParameterSetName = "Restoring")]
        [string]$BackupFolderPath = "C:\ZCMTemp\PathEnumerationBackup",

    [parameter(Mandatory = $False,
        ParameterSetName = "Fixing")]
    [parameter(Mandatory = $False,
        ParameterSetName = "Restoring")]
        [string]$LogName = "C:\ZCMTemp\ServicesFix-3.5.1.Log",
...
if ($RestoreBackup){
    ...
    If ($Passthru){
        If ($Silent -and $(( $Objects | Measure-Object ).Count -ge 1)){
            $True
            Write-Output "Unquoted service path(s) Fixed. Reported at: $(Get-Date)"
            Exit 0
        } ElseIf ($Silent){
            $False
            Write-Output "Unquoted service path(s) not fixed. Reported at: $(Get-Date)"
            Exit 1
        } Else {
            $Objects
        }
